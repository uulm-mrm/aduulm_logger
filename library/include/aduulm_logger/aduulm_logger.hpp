 ...
 */
#if !defined(_ADUULM_LOGGER_H_) and                                            \
    !defined(                                                                  \
        _LOGGER_H_) // Guard aduulm_logger from being included if Defines/Logger
...
using LoggerStreamNameChangeCallback =
    boost::function<void(std::string stream_name)>;
extern std::vector<LoggerInitCallback> g_sublogger_init_callbacks;
extern std::vector<LoggerLevelChangeCallback>
    g_sublogger_level_change_callbacks;
extern std::vector<LoggerStreamNameChangeCallback>
...
#define LOGGER_ROS_EXTRA_DEFINES                                               \
  std::string __attribute__((visibility("hidden"))) g_stream_name =            \
      ROSCONSOLE_DEFAULT_NAME;                                                 \
  const std::array<ros::console::Level, 5> __attribute__(                      \
      (visibility("hidden"))) level_mapping = {                                \
      ros::console::levels::Fatal, ros::console::levels::Error,                \
      ros::console::levels::Warn, ros::console::levels::Info,                  \
      ros::console::levels::Debug};
#else
#define LOGGER_ROS_EXTRA_DEFINES                                               \
  std::string __attribute__((visibility("hidden"))) g_stream_name = "default";
#endif
...
#define DEFINE_LOGGER_VARIABLES                                                \
  ...
  LoggerLevel __attribute__((visibility("hidden"))) g_log_level =              \
      LoggerLevels::Warn;                                                      \
  ...
  std::vector<LoggerInitCallback> __attribute__((visibility("hidden")))        \
      g_sublogger_init_callbacks;                                              \
  std::vector<LoggerLevelChangeCallback> __attribute__((visibility("hidden"))) \
      g_sublogger_level_change_callbacks;                                      \
  std::vector<LoggerStreamNameChangeCallback> __attribute__(                   \
      (visibility("hidden"))) g_sublogger_stream_name_change_callbacks;        \
  LOGGER_ROS_EXTRA_DEFINES                                                     \
  }

#define LOGGER_ADD_SUBLOGGER_LIBRARY(_namespace)                               \
  do {                                                                         \
    aduulm_logger::g_sublogger_init_callbacks.emplace_back(                    \
        _namespace::_initLogger);                                              \
    aduulm_logger::g_sublogger_level_change_callbacks.emplace_back(            \
        _namespace::_setLogLevel);                                             \
    aduulm_logger::g_sublogger_stream_name_change_callbacks.emplace_back(      \
        _namespace::_setStreamName);                                           \
  } while (0)

#define LOGGER_ADD_SUBLOGGER_CLASS(_class, _instance)                          \
  do {                                                                         \
    aduulm_logger::g_sublogger_init_callbacks.emplace_back(                    \
        boost::bind(&_class::_initLogger, _instance));                         \
    aduulm_logger::g_sublogger_level_change_callbacks.emplace_back(            \
        boost::bind(&_class::_setLogLevel, _instance, _1));                    \
    aduulm_logger::g_sublogger_stream_name_change_callbacks.emplace_back(      \
...
#define DEFINE_LOGGER_LIBRARY_INTERFACE_IMPLEMENTATION                         \
  ...
  void _setStreamName(std::string stream_name) {                               \
    ...
    for (auto callback :                                                       \
         aduulm_logger::g_sublogger_stream_name_change_callbacks) {            \
...
__inline__ std::string longTime() {
  ...
  std::time_t _nUSSinceDay =
      std::chrono::duration_cast<std::chrono::microseconds>(
          _aNow - std::chrono::time_point_cast<std::chrono::seconds>(_aNow))
          .count(); // Millisecond fraction

  _nUSSinceDay += _tmBrokenTime.tm_hour * 60ULL * 60 * 1000 *
                  1000; // microseconds for the hours today
  _nUSSinceDay += _tmBrokenTime.tm_min * 60ULL * 1000 *
                  1000; // microseconds for the minutes today
  _nUSSinceDay += _tmBrokenTime.tm_sec * 1000ULL *
                  1000; // microseconds for the seconds today

  int64_t _ntm_us =
      _nUSSinceDay - ((_tmBrokenTime.tm_hour * 3600 +
                       _tmBrokenTime.tm_min * 60 + _tmBrokenTime.tm_sec) *
                      1000000ULL);
  char buf[40];
  sprintf(buf, "%04d-%02d-%02d %02d:%02d:%02d.%06ld",
          _tmBrokenTime.tm_year + 1900, _tmBrokenTime.tm_mon + 1,
          _tmBrokenTime.tm_mday, _tmBrokenTime.tm_hour, _tmBrokenTime.tm_min,
          _tmBrokenTime.tm_sec, _ntm_us);

  return buf;
}
__inline__ std::thread::id thread_id() { return std::this_thread::get_id(); }
} // namespace DataTypesLogger
#define _LOG_BASE                                                              \
  "(" << std::setw(24) << DataTypesLogger::simpleFileName(__FILE__) << ":"     \
      << std::setw(3) << __LINE__ << ") " << std::setw(30) << __FUNCTION__     \
      << "() "
...
#define LOG_FATAL(expr)                                                        \
  ROS_FATAL_STREAM_NAMED(aduulm_logger::g_stream_name,                         \
                         _LOG_BASE << ": " << expr)
#endif

#ifndef LOG_ERR
#define LOG_ERR(expr)                                                          \
  ROS_ERROR_STREAM_NAMED(aduulm_logger::g_stream_name,                         \
                         _LOG_BASE << ": " << expr)
#endif

#ifndef LOG_WARN
#define LOG_WARN(expr)                                                         \
  ROS_WARN_STREAM_NAMED(aduulm_logger::g_stream_name, _LOG_BASE << ": " << expr)
#endif

#ifndef LOG_INF
#define LOG_INF(expr)                                                          \
  ROS_INFO_STREAM_NAMED(aduulm_logger::g_stream_name, _LOG_BASE << ": " << expr)
#endif

#ifndef LOG_DEB
#define LOG_DEB(expr)                                                          \
  ROS_DEBUG_STREAM_NAMED(aduulm_logger::g_stream_name,                         \
                         _LOG_BASE << ": " << expr)
...
#define LOG_FATAL(expr)                                                        \
  do {                                                                         \
    ...
    msg << LOG_MAGENTA << "[FATAL] [" << DataTypesLogger::longTime() << "] "   \
        << _LOG_BASE << ": " << expr << LOG_NORMAL;                            \
    if (aduulm_logger::g_log_to_file) {                                        \
      aduulm_logger::g_oFile << "[FATAL] [" << DataTypesLogger::longTime()     \
                             << "] " << _LOG_BASE << ": " << expr              \
                             << std::endl;                                     \
    }                                                                          \
...
#define LOG_ERR(expr)                                                          \
  do {                                                                         \
    if (aduulm_logger::g_log_level >= aduulm_logger::LoggerLevel::Error) {     \
      ...
      aduulm_logger::CheckLogCnt();                                            \
      std::stringstream msg;                                                   \
      msg << LOG_RED << "[ERROR] [" << DataTypesLogger::longTime() << "] "     \
          << _LOG_BASE << ": " << expr << LOG_NORMAL;                          \
      if (aduulm_logger::g_log_to_file) {                                      \
        aduulm_logger::g_oFile << "[ERROR] [" << DataTypesLogger::longTime()   \
                               << "] " << _LOG_BASE << ": " << expr            \
                               << std::endl;                                   \
      }                                                                        \
      std::cout << msg.str() << std::endl;                                     \
...
#define LOG_WARN(expr)                                                         \
  do {                                                                         \
    if (aduulm_logger::g_log_level >= aduulm_logger::LoggerLevel::Warn) {      \
      ...
      aduulm_logger::CheckLogCnt();                                            \
      std::stringstream msg;                                                   \
      msg << LOG_YELLOW << "[WARN ] [" << DataTypesLogger::longTime() << "] "  \
          << _LOG_BASE << ": " << expr << LOG_NORMAL;                          \
      if (aduulm_logger::g_log_to_file) {                                      \
        aduulm_logger::g_oFile << "[WARN ] [" << DataTypesLogger::longTime()   \
                               << "] " << _LOG_BASE << ": " << expr            \
                               << std::endl;                                   \
      }                                                                        \
      std::cout << msg.str() << std::endl;                                     \
...
#define LOG_INF(expr)                                                          \
  do {                                                                         \
    if (aduulm_logger::g_log_level >= aduulm_logger::LoggerLevel::Info) {      \
      ...
      aduulm_logger::CheckLogCnt();                                            \
      std::stringstream msg;                                                   \
      msg << LOG_GREEN << "[INFO ] [" << DataTypesLogger::longTime() << "] "   \
          << _LOG_BASE << ": " << expr << LOG_NORMAL;                          \
      if (aduulm_logger::g_log_to_file) {                                      \
        aduulm_logger::g_oFile << "[INFO ] [" << DataTypesLogger::longTime()   \
                               << "] " << _LOG_BASE << ": " << expr            \
                               << std::endl;                                   \
        std::cout << msg.str() << std::endl;                                   \
      }                                                                        \
...
#define LOG_DEB(expr)                                                          \
  do {                                                                         \
    if (aduulm_logger::g_log_level >= aduulm_logger::LoggerLevel::Debug) {     \
      ...
      aduulm_logger::CheckLogCnt();                                            \
      std::stringstream msg;                                                   \
      msg << LOG_BLUE << "[DEBUG] [" << DataTypesLogger::longTime() << "] "    \
          << _LOG_BASE << ": " << expr << LOG_NORMAL;                          \
      if (aduulm_logger::g_log_to_file) {                                      \
        aduulm_logger::g_oFile << "[DEBUG] [" << DataTypesLogger::longTime()   \
                               << "] " << _LOG_BASE << ": " << expr            \
                               << std::endl;                                   \
      }                                                                        \
      std::cout << msg.str() << std::endl;                                     \
...
#define LOG_TRACE(expr)                                                        \
  do {                                                                         \
    std::lock_guard<std::mutex> _oLockLogger(aduulm_logger::g_oLoggerMutex);   \
    aduulm_logger::CheckLogCnt();                                              \
    std::cout << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex     \
              << DataTypesLogger::thread_id() << std::dec << ")" << LOG_BLUE   \
              << std::setw(9) << " TRACE " << LOG_NORMAL << _LOG_BASE << ": "  \
              << LOG_BLUE << expr << LOG_NORMAL << std::endl;                  \
    aduulm_logger::g_oFile << "[" << DataTypesLogger::longTime() << "] (0x"    \
                           << std::hex << DataTypesLogger::thread_id()         \
                           << std::dec << ")" << std::setw(9) << " TRACE "     \
...
#define LOG_VERB(expr)                                                         \
  do {                                                                         \
    std::lock_guard<std::mutex> _oLockLogger(aduulm_logger::g_oLoggerMutex);   \
    LOG_WARN(                                                                  \
        "Use of LOG_VERB() is deprecated, please use LOG_DEB() instead.");     \
    aduulm_logger::CheckLogCnt();                                              \
    std::cout << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex     \
              << DataTypesLogger::thread_id() << std::dec << ")"               \
              << std::setw(9) << " VERBOSE " << _LOG_BASE << ": " << expr      \
              << std::endl;                                                    \
    aduulm_logger::g_oFile << "[" << DataTypesLogger::longTime() << "] (0x"    \
                           << std::hex << DataTypesLogger::thread_id()         \
                           << std::dec << ")" << std::setw(9) << " VERBOSE "   \
                           << _LOG_BASE << ": " << expr << std::endl;          \
  } while (0)
#endif
#else
...
#define CHECK_IF_IS_EXPRESSION(VAR)                                            \
  std::char_traits<char>::length(#VAR) > 0 && #VAR[0] == '"' &&                \
      #VAR[std::char_traits<char>::length(#VAR) - 1] == '"'
#endif

#ifndef STRINGIFY_VAR
#include <iostream>
namespace LOGGER_H_INTERNAL {
template <class T> inline std::string stringify(T arg) {
  std::stringstream ss;
  ss << arg;
  return ss.str();
}

template <> inline std::string stringify(std::string arg) {
  std::stringstream ss;
  ss << "'" << arg << "'";
  return ss.str();
}

template <> inline std::string stringify(const char *arg) {
  std::stringstream ss;
  ss << "'" << arg << "'";
  return ss.str();
}
...
template <class T>
std::string stringify(bool isLiteral, std::string name, T arg,
...
template <typename _Scalar, int _Rows, int _Cols, int _Options, int _MaxRows,
          int _MaxCols>
inline std::string stringify(
    bool isLiteral, std::string name,
    Eigen::Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols> arg,
    bool isLastValue = false) {
  std::stringstream ss;
  if (isLiteral)
...
template <class T>
inline std::string stringify(bool isLiteral, std::string name, T arg,
                             bool isLastValue) {
  std::stringstream ss;
  if (isLiteral)
    ss << arg << " ";
...
#define STRINGIFY_VAR(VAR)                                                     \
  LOGGER_H_INTERNAL::stringify(CHECK_IF_IS_EXPRESSION(VAR), #VAR, VAR)
#define STRINGIFY_VAR_LAST(VAR)                                                \
  LOGGER_H_INTERNAL::stringify(CHECK_IF_IS_EXPRESSION(VAR), #VAR, VAR, true)
#endif // STRINGIFY_VAR
...
#ifndef LOG_X_VAR_2
#define LOG_X_VAR_2(macroname, var1, var2)                                     \
  macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR_LAST(var2));
#endif

#ifndef LOG_X_VAR_3
#define LOG_X_VAR_3(macroname, var1, var2, var3)                               \
  macroname(STRINGIFY_VAR(var1)                                                \
            << STRINGIFY_VAR(var2) << STRINGIFY_VAR_LAST(var3));
#endif

#ifndef LOG_X_VAR_4
#define LOG_X_VAR_4(macroname, var1, var2, var3, var4)                         \
  macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3)  \
                                << STRINGIFY_VAR_LAST(var4));
#endif

#ifndef LOG_X_VAR_5
#define LOG_X_VAR_5(macroname, var1, var2, var3, var4, var5)                   \
  macroname(STRINGIFY_VAR(var1)                                                \
            << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3)                      \
            << STRINGIFY_VAR(var4) << STRINGIFY_VAR_LAST(var5));
#endif

#ifndef LOG_X_VAR_6
#define LOG_X_VAR_6(macroname, var1, var2, var3, var4, var5, var6)             \
  macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3)  \
                                << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5)  \
                                << STRINGIFY_VAR_LAST(var6));
#endif
...
#define LOG_X_VAR_7(macroname, var1, var2, var3, var4, var5, var6, var7)       \
  macroname(STRINGIFY_VAR(var1)                                                \
            << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3)                      \
            << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5)                      \
            << STRINGIFY_VAR(var6) << STRINGIFY_VAR_LAST(var7));
#endif
...
#define LOG_X_VAR_8(macroname, var1, var2, var3, var4, var5, var6, var7, var8) \
  macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3)  \
                                << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5)  \
                                << STRINGIFY_VAR(var6) << STRINGIFY_VAR(var7)  \
                                << STRINGIFY_VAR_LAST(var8));
#endif

#ifndef LOG_X_VAR_9
#define LOG_X_VAR_9(macroname, var1, var2, var3, var4, var5, var6, var7, var8, \
                    var9)                                                      \
  macroname(STRINGIFY_VAR(var1)                                                \
            << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3)                      \
            << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5)                      \
...
#ifndef LOG_X_VAR_10
#define LOG_X_VAR_10(macroname, var1, var2, var3, var4, var5, var6, var7,      \
                     var8, var9, var10)                                        \
  macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3)  \
                                << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5)  \
                                << STRINGIFY_VAR(var6) << STRINGIFY_VAR(var7)  \
                                << STRINGIFY_VAR(var8) << STRINGIFY_VAR(var9)  \
                                << STRINGIFY_VAR_LAST(var10));
#endif
...
#define GET_RIGHT_MACRO_OVERLOAD(_1, _2, _3, _4, _5, _6, _7, _8, _9, _10,      \
                                 NAME, ...)                                    \
  NAME
#endif
...
#define LOG_X_VAR(MACRONAME, ...)                                              \
  GET_RIGHT_MACRO_OVERLOAD(__VA_ARGS__, LOG_X_VAR_10, LOG_X_VAR_9,             \
                           LOG_X_VAR_8, LOG_X_VAR_7, LOG_X_VAR_6, LOG_X_VAR_5, \
...
#if defined(IS_ROS) || defined(USE_ROS_LOG)
  if (log_level >=
      LoggerLevels::None) // should always be true, but just in case
  {
    ...
    if (ros::console::set_logger_level(
            std::string(ROSCONSOLE_DEFAULT_NAME) + "." + g_stream_name, lvl)) {
...
static inline void setStreamName(std::string name) { g_stream_name = name; }

static inline bool initLogger(std::string file_name,
                              LoggerLevel log_level = DEFAULT_LOG_LEVEL) {
