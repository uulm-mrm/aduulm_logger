...
#define _LOG_BASE "(" << std::setw(24) << DataTypesLogger::simpleFileName(__FILE__) \
...
#ifndef LOG_ERR
#define LOG_ERR(expr) { if (g_log_level >= 1) {std::lock_guard<std::mutex> _oLockLogger(g_oLoggerMutex); CheckLogCnt(); std::cout << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
		LOG_RED << std::setw(9) << " ERROR " << LOG_NORMAL << _LOG_BASE << ": " << LOG_RED << expr << LOG_NORMAL << std::endl; \
		g_oFile << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
				std::setw(9) << " ERROR " << _LOG_BASE << ": " << expr << std::endl;} }
#endif

#ifndef LOG_WARN
#define LOG_WARN(expr) { if (g_log_level >= 2) {std::lock_guard<std::mutex> _oLockLogger(g_oLoggerMutex); CheckLogCnt(); std::cout << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
		LOG_YELLOW << std::setw(9) << " WARNING " << LOG_NORMAL << _LOG_BASE << ": " << LOG_YELLOW << expr << LOG_NORMAL << std::endl; \
		g_oFile << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
...
#ifndef LOG_INF
#define LOG_INF(expr) { if (g_log_level >= 3) {std::lock_guard<std::mutex> _oLockLogger(g_oLoggerMutex); CheckLogCnt(); std::cout << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
		LOG_GREEN << std::setw(9) << " INFO " << LOG_NORMAL << _LOG_BASE << ": " << LOG_GREEN << expr << LOG_NORMAL << std::endl; \
		g_oFile << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
...
#ifndef LOG_DEBUG
#define LOG_DEB(expr) { if (g_log_level >= 4) {std::lock_guard<std::mutex> _oLockLogger(g_oLoggerMutex); CheckLogCnt(); std::cout << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
		LOG_CYAN << std::setw(9) << " DEBUG " << LOG_NORMAL << _LOG_BASE << ": " << LOG_CYAN << expr << LOG_NORMAL << std::endl; \
		g_oFile << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
...
#ifdef LOG_TRACEING
	#ifndef LOG_TRACE
	#define LOG_TRACE(expr) { std::lock_guard<std::mutex> _oLockLogger(g_oLoggerMutex); CheckLogCnt(); std::cout << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
		LOG_BLUE << std::setw(9) << " TRACE " << LOG_NORMAL << _LOG_BASE << ": " << LOG_BLUE << expr << LOG_NORMAL << std::endl; \
		g_oFile << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
...
#ifdef LOG_VERBOSE
  #ifndef LOG_VERB
  #define LOG_VERB(expr) { std::lock_guard<std::mutex> _oLockLogger(g_oLoggerMutex); CheckLogCnt(); std::cout << "[" << DataTypesLogger::longTime() << "] (0x" << std::hex << DataTypesLogger::thread_id() << std::dec << ")" << \
...
#ifndef PROGRESS
#define PROGRESS(_PRGS) const int _BARWidth = 70; std::cout << LOG_BOLDBLUE << "["; int pos = _BARWidth * _PRGS; for (int i = 0; i < _BARWidth; ++i) { if (i < pos) std::cout << "="; else if (i == pos) std::cout << ">"; else std::cout << " "; } std::cout << std::setprecision(2) <<"] " << int(_PRGS * 100.0) << " %\r"; std::cout.flush(); std::cout << LOG_NORMAL;
#endif
...
// create a new line before a matrix
  template<typename _Scalar, int _Rows, int _Cols, int _Options, int _MaxRows, int _MaxCols>
  inline std::string stringify(bool isLiteral, std::string name, Eigen::Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols> arg, bool isLastValue=false)
  {
    std::stringstream ss;
    if (isLiteral)
      ss << arg << " ";
...
#ifndef LOG_X_VAR_1
#define LOG_X_VAR_1(macroname, var)\
  macroname(STRINGIFY_VAR_LAST(var) );
#endif
...
#define LOG_X_VAR_5(macroname, var1, var2, var3, var4, var5)\
    macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3) << STRINGIFY_VAR(var4) << STRINGIFY_VAR_LAST(var5));
#endif

#ifndef LOG_X_VAR_6
#define LOG_X_VAR_6(macroname, var1, var2, var3, var4, var5, var6)\
    macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3) << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5) << STRINGIFY_VAR_LAST(var6));
#endif

#ifndef LOG_X_VAR_7
#define LOG_X_VAR_7(macroname, var1, var2, var3, var4, var5, var6, var7)\
    macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3) << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5) << STRINGIFY_VAR(var6) << STRINGIFY_VAR_LAST(var7));
#endif

#ifndef LOG_X_VAR_8
#define LOG_X_VAR_8(macroname, var1, var2, var3, var4, var5, var6, var7, var8)\
    macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3) << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5) << STRINGIFY_VAR(var6) << STRINGIFY_VAR(var7) << STRINGIFY_VAR_LAST(var8));
#endif

#ifndef LOG_X_VAR_9
#define LOG_X_VAR_9(macroname, var1, var2, var3, var4, var5, var6, var7, var8, var9)\
    macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3) << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5) << STRINGIFY_VAR(var6) << STRINGIFY_VAR(var7) << STRINGIFY_VAR(var8) << STRINGIFY_VAR_LAST(var9));
#endif

#ifndef LOG_X_VAR_10
#define LOG_X_VAR_10(macroname, var1, var2, var3, var4, var5, var6, var7, var8, var9, var10)\
    macroname(STRINGIFY_VAR(var1) << STRINGIFY_VAR(var2) << STRINGIFY_VAR(var3) << STRINGIFY_VAR(var4) << STRINGIFY_VAR(var5) << STRINGIFY_VAR(var6) << STRINGIFY_VAR(var7) << STRINGIFY_VAR(var8) << STRINGIFY_VAR(var9) <<  STRINGIFY_VAR_LAST(var10));
#endif
...
#ifndef LOG_X_VAR
#define LOG_X_VAR(MACRONAME, ...) GET_RIGHT_MACRO_OVERLOAD(__VA_ARGS__, LOG_X_VAR_10, LOG_X_VAR_9, LOG_X_VAR_8, LOG_X_VAR_7, LOG_X_VAR_6, LOG_X_VAR_5, LOG_X_VAR_4, LOG_X_VAR_3, LOG_X_VAR_2, LOG_X_VAR_1)(MACRONAME, __VA_ARGS__)
#endif
